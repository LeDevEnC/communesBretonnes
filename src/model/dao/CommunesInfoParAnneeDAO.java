...
public class CommunesInfoParAnneeDAO extends DAO<CommunesInfoParAnneeDAO>{
    ...
    public CommunesInfoParAnnee findByIDComplete(Long annee, CommuneBase communeBase) {
        ...
        try (Connection connection = getConnection();
             PreparedStatement statement = connection.prepareStatement("SELECT * FROM DonneesAnnuelles WHERE laCommune = ? AND lAnnee = ?")) {
            statement.setLong(1, communeBase.getIdCommune());
            statement.setLong(2, annee);
            try (ResultSet resultSet = statement.executeQuery()) {
                while (resultSet.next()) {
                    communesInfoParAnnee = new CommunesInfoParAnnee(communeBase, anneeS, resultSet.getInt("nbMaison"), resultSet.getInt("nbAppart"), resultSet.getDouble("prixMoyen"), resultSet.getInt("prixM2Moyen"), resultSet.getDouble("SurfaceMoy"), resultSet.getDouble("depensesCulturellesTotales"), resultSet.getDouble("budgetTotal"), resultSet.getInt("population"));
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public CommunesInfoParAnneeDAO findByID(Long id) {
        throw new UnsupportedOperationException("Méthode non implémentée car il n'y a pas de clé satisfaisante pour identifier des données annuelles dans notre représentation");
    }
