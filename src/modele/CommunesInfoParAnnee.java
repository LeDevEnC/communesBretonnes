package modele;
public class CommunesInfoParAnnee {
    private int nbMaison;
    private int nbAppart;
    private double prixMoyen;
    private int prixMCarreMoyen;
    private double surfaceMoy;
    private double depCulturellesTotales;
    private double budgetTotal;
    private double population;
    private CommmuneBase laCommune;
    private Annee lAnnee;

    public CommunesInfoParAnnee(CommuneBase laCommune, Annee lAnnee, int nbMaison, int nbAppart, double prixMoyen, double prixMCarreMoyen, double surfaceMoy, double depCulturellesTotales, double budgetTotal, int population){
    
    }

    public int getNbMaison(){

    }
    public int getNbAppart(){

    }
    public double getPrixMoyen(){

    }
    public double getPrixMCarreMoyen(){

    }
    public double getSurfaceMoy(){

    }
    public double getDepCulturellesTotales(){

    }
    public double getBudgetTotal(){

    }
    public double getPopulation(){

    }
    public Commune getLaCommune(){

    }
    public Annee getLannee(){

    }
    public void setNbMaison(int nbMaison){

    }
    public void setNbAppart(int nbAppart){

    }
    public void setPrixMoyen(double prixMoyen){

    }
    public void setPrixMCarreMoyen(double prixMCarreMoyen){

    }
    public void setSurfaceMoy(double surfaceMoy){

    }
    public void setDepCulturellesTotales(double depCulturellesTotales){

    }
    public void setBudgetTotal(double budgetTotal){

    }
    public void setPopulation(double population){

    }
    public String toString(){

    }
    public double compareNbHabitants(CommunesInfoParAnnee uneAutreCommune){

    }

}